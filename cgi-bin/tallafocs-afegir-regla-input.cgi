...
resultat=$(
    "$CMD" tallafocs configurar afegir_nova_regla_input \
        "$VNOM" "$POSICIO" "$REGLA_COMPLETA" 2>&1
)
...
if [ -n "$retorn" ]; then
    echo "<br>"
    echo "<a href=\"$retorn?id=$ID&vnom=$VNOM\"><button>Tornar</button></a>"
fi
